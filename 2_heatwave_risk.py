...
# 2. Abrir el NetCDF de temperaturas EURO-CORDEX descargado en el Paso 1 (ámbito nacional,
# ya recortado a España y al año 2030). Trae la media de verano de la temperatura máxima
# y de la mínima, ambas en grados Celsius, a ~11km de resolución, en lat/lon normales.
ds_temp = xr.open_dataset("temperaturas_2030_eurocordex.nc", engine="netcdf4")
ds_temp = ds_temp.rio.write_crs("EPSG:4326")
ds_temp = ds_temp.rio.set_spatial_dims(x_dim="lon", y_dim="lat", inplace=True)

# 3. Abrir el NetCDF de humedad relativa (CORDEX crudo, ver README sobre la inconsistencia
...
ds_hum = xr.open_dataset("humedad_2030_eurocordex.nc", engine="netcdf4")
...
# 4. Extraer, para cada municipio, la temperatura máxima y mínima media de verano y la
...
# 5. Índice de calor (temperatura + humedad) para el día (con temp. máxima) y para la noche
# (con temp. mínima, ligado a las "noches tropicales"): a igual temperatura, más humedad
# hace que el calor se sienta -y afecte al cuerpo- más de lo que indica el termómetro.
...
# 6. Obtener población por municipio (INE, Padrón - último año disponible)
resp = requests.get(
...
# 7. Indicador de riesgo de mortalidad por calor (proxy relativo, no una predicción real de
# muertes). La exposición al calor combina, a partes iguales, el índice de calor diurno y el
# nocturno (calor + humedad) - las dos variables climáticas pesan 50%/50% entre sí. Esa
# exposición combinada se pondera después a partes iguales con la población expuesta (en
# escala logarítmica, muy sesgada: de ~10 a >3M habitantes por municipio), para reflejar
...
# Guardar el indicador de riesgo de ola de calor por municipio
municipios.to_file("municipios_heatwave_risk.geojson", driver="GeoJSON")
print("Zonificación de riesgo de ola de calor finalizada.")
