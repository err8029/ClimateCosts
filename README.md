...
Below the maps, two tables: **top 10 municipalities by 2030→2050 increment** for the selected scenario/variable, and **top 10 Spanish cities by population** (fixed list — Madrid, Barcelona, València, Zaragoza, Sevilla, Málaga, Murcia, Palma, Alacant/Alicante, Bilbao — since population doesn't vary by year/scenario in this project, so the ranking wouldn't either) showing the same variable's 2030/2050 values and increment, since a city might not show up in the "biggest increment" list even though it matters more in absolute terms. Both are joined by `ine_code`, not name: 17 municipality names repeat nationally (e.g. two different "Mieres"), so a name-only join would silently merge unrelated municipalities.

**Known limitation**: `heat_mortality_risk` is min-max normalized *independently* for each year/scenario, so its "increment" isn't purely climate change — a municipality's normalized score can dip even as its actual heat exposure rises, if *other* municipalities' exposure grew faster and shifted the national 0–1 scale. The raw °C Heat Index columns don't have this issue (they're physical units, directly comparable across years).
